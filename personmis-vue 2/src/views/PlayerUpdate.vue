            ...
            <el-row>
                ...
                <el-col :span="8">
                    <el-form-item label="部门" prop="depart_id">
                        <el-select v-model="addForm.depart_id" placeholder="请选择部门">
                            <el-option v-for="(item, index) in dparts" :key="index" :label="item.dname"
                ...
                <el-col :span="8">
                    <el-form-item label="球员位置(岗位)" prop="post_id">
                        <el-select v-model="addForm.post_id" placeholder="请选择球员位置">
                ...
                <el-col :span="8">
                    <el-form-item label="进球数" prop="score">
                        <el-input v-model="addForm.score" placeholder="请输入进球数"></el-input>
                    </el-form-item>
                </el-col>
            ...
            <el-form-item label="毕业日期"  prop="graduationdate">
                ...
                <el-col :span="8">
                    <el-form-item label="试用期开始日期" prop="startdate">
                        <el-date-picker v-model="addForm.startdate" type="date" placeholder="选择参加工作日期"
                            style="width:100%"></el-date-picker>
                    </el-form-item>
                </el-col>
                <el-col :span="8">
                    <el-form-item label="试用期结束日期" prop="enddate">
                        <el-date-picker v-model="addForm.enddate" type="date" placeholder="选择参加工作日期"
                            style="width:100%"></el-date-picker>
                    </el-form-item>
                </el-col>
                <el-col :span="8">
                    <el-form-item>
                        <el-button type="primary" @click="update(addForm)"
                ...
                <el-col :span="8">
                    <el-form-item label="最高学历" prop="education">
                        <el-select v-model="addForm.education">
                            <el-option v-for="(item, index) in educations" :key="index" :label="item"
                                :value="item"></el-option>
                        </el-select>
                    </el-form-item>
                </el-col>
...
export default {
    ...
    methods: {
        ...
        loadStaffs() {
            this.$axios
                .get('/getPlayerCoachByPage?currentPage=' + this.currentPage + '&&pageSize=' + this.pageSize)
                .then(successResponse => {
                    alert(JSON.stringify(successResponse.data.players))
                    this.tableData = successResponse.data.players
        ...
        selectStaffsByCon() {
            ...
            this.$axios
                .post('/selectPlayerCoachByCon', this.selectForm)//直接提交表单
                .then(successResponse => {
                    this.tableData = successResponse.data.players
                    this.total = successResponse.data.total
                })
        ...
        handleEdit(index, row, act) {
            ...
            this.$axios
                .get('/getPlayerCoachDetail?id=' + row.id)
                .then(successResponse => {
        ...
        handleDelete(index, row) {
            ...
            this.$confirm('删除员工, 是否继续?', '提示', {
                ...
                this.$axios
                    .post('/deletePlayerCoach?id=' + row.id)
                    .then(successResponse => {
        ...
        update() {
            ...
            this.$axios
                .post('/updatePlayerCoach', this.addForm)//直接提交表单
                .then(successResponse => {
                    if (successResponse.data === "ok") {
