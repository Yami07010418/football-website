<template>
  <el-dialog title="教练员登录" v-model="dialogVisible" width="30%">
    <div class="box">
        ...
        if (valid) {
          ...
          this.$axios
            ...
            .then(successResponse => {
              if (successResponse.data.msgId === "A001") {
                // ElMessage.error('该邮箱没注册！')
              } else if (successResponse.data.msgId === "A002") {
                ...
                sessionStorage.setItem("aname", successResponse.data.result.aname);
                sessionStorage.setItem("bid", successResponse.data.result.bid);
                this.$alert(sessionStorage.getItem("aname"))
                let path = this.$route.query.redirect
                this.$router.replace({path: path === '/' || path === undefined ? '/addplan': path})
              }
