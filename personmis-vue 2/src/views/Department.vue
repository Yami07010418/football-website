...
export default {
  ...
  //Vue.js组件的created生命周期钩子函数。在Vue.js中，created钩子函数会在实例创建完成后被调用。
  data() {
    return {
      dtypes: ['','主席','足球运动员', '教练', '技术分析师', '球探', '后勤'],
      selectForm: {
        currentPage: 1,
        pageSize: 1,
        act: ''
...
//表示使用GET方法发送一个HTTP请求，请求的目标地址是/getDepartmentByPage，并传递了两个参数：currentPage和pageSize。
      this.$axios
        ...
        .then(successResponse => {
          ...
          console.log("data:" + successResponse.data.departs)
    ...
    handleCurrentChange(val) {
      this.currentPage = val//即将当前页码更新为新的值。
    ...
    handleEdit(index, row, act) {
      ...
      this.$axios
        .get('/getDepartmentDetail?id=' + row.id)//当前行数据对象的id值
        .then(successResponse => {
          // alert(JSON.stringify(successResponse.data.aDepart))
          // alert(JSON.stringify(successResponse.data.departs))
      ...
      this.$axios
        ...
        .then(successResponse => {
          if (successResponse.data === "ok") {
            ...
            //如果成功响应的数据为字符串"ok"，则使用this.$alert方法弹出一个提示框，显示"修改成功"的消息，并设置this.dialogVisibleDetail为false，关闭对话框。
